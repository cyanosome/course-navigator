...
flowchart TD
    ...
    P4["Phase 4: CS2023 オントロジー統合 & Web教材 Ingestion<br/>(CS2023実データマッピング / 開志創造大連携・講義詳細データ検証 / Web教材 / 多層化)"]
...
- **主なタスク・実験項目**:
  ...
  - **（先行準備・並行検討）タスク 3-5: 開志創造大学との共同研究テーマ設計およびデータアクセス準備（交渉中）**
    - Phase 4 に向け、教育系教員との意見交換や、授業スライド・字幕データの提供に向けたデータアクセス・倫理的配慮の体制構築を Phase 3 と並行（パラレル）して推進する。

---

### Phase 4: CS2023 基準オントロジー統合 & Web教材 Ingestion 【未着手】

- **目的**: 単一大学でのプロトタイプ成立後、本来の目標である **国際標準 ACM/IEEE CS2023 オントロジー** を Neo4j に投入。大学の科目をオントロジーへ自動マッピングし、さらに学内でカバーできない領域を補完する学外 Web 公開教材（OCW / MOOCs 等）を取り込んで多層ナレッジグラフを完成させる。また、オンライン講義資産が豊富な **開志創造大学との共同研究連携（交渉中）** を通じて、シラバスのテキスト情報のみに依存しないマルチモーダルな講義詳細データ（字幕・スライド等）を活用したオントロジーマッチング高度化と実運用フィードバックの獲得を検証する。
- **開志創造大学との連携背景（交渉中）**:
  - **連携形態**: 開志創造大学側から「Course Navigatorを利用する研究テーマ」として参加する形をとり、双方にとって活動しやすい適度な自律性を担保した共同研究体制の構築を協議中。
  - **オンライン授業データの活用**: 開志創造大学の最大の強みである蓄積されたオンライン講義資産（字幕データ、講義スライド、教科書情報など）にアクセス可能な体制を教員主導で検討中。
  - **課題認識と仮説**: シラバスの記載（概要や到達目標）のみでは講義の実質的内容や解像度が不足し、CS2023 とのマッピング精度に限界が生じる可能性がある。字幕やスライド等の詳細データを活用することで、知識単位との適合判定が大幅に高度化できるかを検証する。
- **主なタスク・実験項目**:
  - **実験 4-1: CS2023 オントロジーのグラフモデリングと投入**
    - KA / KU / Topic / LO の階層ノードを Neo4j に投入。
  - **実験 4-2: LLM による学内科目 ↔ CS2023 オントロジーの自動推論マッピング**
    - 大学シラバスの内容から、LLM を用いて CS2023 のどの知識領域・トピックに対応するかを推論し、自動リンク (`[:MAPS_TO]`) を構築。
  - **実験 4-3: 外部公開 Web 教材の収集とオントロジーマッピング**
    - 大学公開講座 (OCW) や MOOCs を収集し、CS2023 のトピックに事前グラフ化して接続（大学内にない科目の補強）。
  - **実験 4-4: 開志創造大学との連携による詳細講義データ（字幕・スライド等）を活用したオントロジーマッチング高度化および実運用フィードバック実験（交渉中）**
    - **詳細講義データの活用・マッチング比較**: 開志創造大学のオンライン授業資産（講義字幕・スライド・教科書等）を取り込み、シラバス単体の場合と比べて CS2023 とのマッピング精度・解像度がどの程度向上するかを比較検証する。
    - **外部Web教材推薦との連動**: 詳細講義データの解析により学内カリキュラムで不足しているトピックを特定し、実験 4-3 で構築した外部Web教材（OCW/MOOCs等）を適切に補完推薦できるか実証する。
    - **実運用テストとフィードバック収集**: 開志創造大学の学生を対象にプロトタイプを実運用展開し、アクセスログ・対話ログの分析、およびアンケートやインタビューによる定性・定量フィードバックを収集して推薦の納得感や有用性を評価する。
    - **教員連携による教育的評価**: 開志創造大学の教育系教員等と連携し、教育的観点からカリキュラム整合性やシステムの実用性についての評価・知見を得る。
  - **タスク 4-5: プロダクト機能の本格拡充**
